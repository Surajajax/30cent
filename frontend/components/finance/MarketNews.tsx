...
export default function MarketNews() {
  ...
  async function fetchNews(showRefresh = false) {
    try {
      ...
      /*
       * IMPORTANT:
       * Don't hide the backend error.
       * Read the actual response from FastAPI.
       */
      if (!response.ok) {
        let detail = "";
        ...
        throw new Error(
          detail ||
            `Market news request failed (${response.status})`
        );
      }
      ...
      if (!Array.isArray(result?.data)) {
        console.error(
          "Unexpected market news response:",
          result
        );

        throw new Error(
          "Invalid market news response from backend"
        );
      }
        ...
        <div className="rounded-2xl border border-[#7c443b] bg-[#3a211e] p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs leading-5 text-[#c98980]">
                {error}
              </p>
            </div>
            ...
            <button
              ...
              onClick={() => void fetchNews(true)}
              disabled={refreshing}
              className="shrink-0 rounded-lg border border-[#7c443b] px-3 py-2 text-xs font-medium text-[#f2a092] transition hover:bg-[#4a2925] disabled:cursor-not-allowed disabled:opacity-50"
            >
              {refreshing
                ? "Retrying..."
                : "Retry"}
        ...
        news.length === 0 && (
          <div className="rounded-2xl border border-[#2a2d29] bg-[#181b18] p-10 text-center">
            ...
            <button
              ...
              onClick={() => void fetchNews(true)}
              disabled={refreshing}
              className="mt-5 rounded-xl border border-[#2a2d29] bg-[#20241f] px-4 py-2.5 text-sm font-medium text-[#858a83] transition hover:border-[#383d36] hover:text-[#f4f2ed] disabled:cursor-not-allowed disabled:opacity-50"
            >
              {refreshing
                ? "Refreshing..."
                : "Try again"}
